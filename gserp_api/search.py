...
class Search:
    """
    Class for performing search with specified parameters.
    """

    def __init__(self, gateway: ApiGateway, params):
        ...
        self.gateway: ApiGateway = gateway
    
    def search(self, query: str, *fields, country=None, lang="en", location=None, lang_restrict=None):
        """
        ...
        if "position" in fields:
            return (
                self._format_json_output({"error": "The 'position' key is not a valid choice."}),
                None,
            )
        invalid_keys = set(fields) - VALID_KEYS
        if invalid_keys:
            return (
                self._format_json_output({"error": f"Invalid keys: {', '.join(invalid_keys)}"}),
                None,
            )
        ...
        session.mount("https://www.google.com", self.gateway)
        ...
        try:
            ...
            if response.status_code != 200:
                return self._format_json_output({"error": "Failed to retrieve web page."})
            ...
            for result in soup.select(".tF2Cxc"):
                ...
                if fetch_link:
                    ...
                    if link_element and link_element.has_attr("href"):
                        result_dict["link"] = self._clean_url(link_element["href"])
                ...
                if fetch_snippet:
                    result_dict["snippet"] = self._extract_snippet(result)
            ...
            return self._format_json_output(json_result)
    ...
    def _format_json_output(self, data):
        """
    ...
    def _extract_snippet(self, result_item):
        """
